...
# export var floorgen_y_min: int= 10
# export var floorgen_y_max: int= 18

# export var ceilgen_y_min: int= -8
# export var ceilgen_y_max: int= -5
...
# func generate_bool(chance: int) -> bool:
# 	var num = int(rand_range(1, chance))
# 	if num == 1:
# 		return true
# 	return false
...
# 		var should_make_map := generate_bool(3)
# 		if x % 48 == 0 and should_make_map and x != 0 and last_gen_x != x:
...
# 			var rand_check := generate_bool(10)
...
# 		var should_make_ammo_refill := generate_bool(1)
# 		if should_make_ammo_refill:
# 			print(x)
# 			place_speed(Vector2(x, 0))
